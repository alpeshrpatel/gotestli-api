...
const UserPurchases = function (userPurchases) {
  ...
  this.ins_id = userPurchases.ins_id;
...
UserPurchases.getMyPurchases = async (id, result) => {
    connection.query(
      `SELECT * FROM user_purchases wq JOIN question_set qs ON wq.questionset_id = qs.id WHERE user_id = ${id} ORDER BY wq.created_date DESC;
  `,
      (err, res) => {
        if (err) {
